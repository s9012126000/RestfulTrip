...
def get_hotel_data():
    ...
    cursor.execute(
        "SELECT date, tol, resource FROM dash_hotels ORDER BY date DESC LIMIT 15"
    )
    ...
    tol = [
        hotels["tol"][i] + booking["tol"][i] + agoda["tol"][i]
        for i in range(len(hotels["date"]))
    ]
    hotel_pack = {"hotel": hotels, "booking": booking, "agoda": agoda, "tol": tol}
    return hotel_pack
...
def get_crawler_time():
    ...
    cursor = mysql_db.cursor()
    cursor.execute(
        "SELECT * FROM dash_time WHERE pipe = 1 AND end IS NOT null ORDER BY date DESC LIMIT 5"
    )
    pipe1_time = cursor.fetchall()
    pipe1_time = sorted(pipe1_time, key=lambda x: x["date"])
    mysql_db.commit()
    cursor.execute(
        "SELECT * FROM dash_time WHERE pipe = 2 AND end IS NOT null ORDER BY date DESC LIMIT 15"
    )
    ...
    time_pack = {
        "pipe1": {
            "date": [str(x["date"]) for x in pipe1_time],
            "spend": [
                round((x["end"] - x["start"]).seconds / 3600, 3) for x in pipe1_time
            ],
        },
        "pipe2": {
            "hotel": {
                ...
                "spend": [
                    round((x["end"] - x["start"]).seconds / 3600, 3)
                    for x in pipe2_time
                    if x["resource"] == 1
                ],
            },
...
def get_hotel_accuracy():
    ...
    accu_pack = {
        "hotel": {
            ...
            "accu": [
                round(((x["tol"] - x["hotel_err"]) / x["tol"]) * 100, 2)
                for x in accu
                if x["resource"] == 1
            ],
        },
        "booking": {
            ...
            "accu": [
                round(((x["tol"] - x["booking_err"]) / x["tol"]) * 100, 2)
                for x in accu
                if x["resource"] == 2
            ],
        },
        "agoda": {
            ...
            "accu": [
                round(((x["tol"] - x["agoda_err"]) / x["tol"]) * 100, 2)
                for x in accu
                if x["resource"] == 3
            ],
        },
        "total": {
            "date": [str(x["date"]) for x in accu if x["resource"] == 1],
            "accu": [
                round(((x["tol_num"] - x["repeat_num"]) / x["tol_num"]) * 100, 2)
                for x in accu
                if x["resource"] == 1
            ],
        },
