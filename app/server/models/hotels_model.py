...
def get_hotel_price(checkin, checkout, hid, person):
    ...
    for price in prices:
        if price['resource'] == 1:
            price['url'] = price['url'].replace('chkin=2022-10-01', f'chkin={checkin}') \
                .replace('chkout=2022-10-02', f'chkout={checkout}')
        elif price['resource'] == 2:
            price['url'] = price['url'].replace('checkin=2022-06-18', f'checkin={checkin}') \
            ...
            price['url'] = price['url'].replace('checkIn=2022-06-28', f'checkIn={checkin}') \
    ...
    for p in price_sum.values():
        try:
            prices[p['hotel_id']][p['resource']] = (format(int(p['price'] / p['count']), ','), p['url'])
        except KeyError:
            prices[p['hotel_id']] = {}
            prices[p['hotel_id']][p['resource']] = (format(int(p['price'] / p['count']), ','), p['url'])
    return prices