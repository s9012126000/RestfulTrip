...
class Worker(threading.Thread):
    ...
    def get_region_hotels(self, div):
        ...
        WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(
                (By.XPATH, "//li[@data-selenium='autosuggest-item']")
            )
        ).click()
        time.sleep(1)
        ...
        self.driver.find_element(
            By.XPATH, "//button[@data-selenium='searchButton']"
        ).click()
        try:
            WebDriverWait(self.driver, 10).until(
                ec.element_to_be_clickable(
            ...
            def get_cards():
                ...
                card = WebDriverWait(self.driver, 10).until(
                    ec.presence_of_all_elements_located(
                        (By.XPATH, "//a[@class='PropertyCard__Link']")
                    )
                )
                card = [
            ...
            for c in range(len(cards)):
                ...
                def fetching():
                    ...
                    try:
                        rating = wait.until(
                            ec.presence_of_element_located(
                                (
                                    By.XPATH,
                                    "//div[@class='ReviewScoreCompact__section']",
                                )
                            )
                        ).text
                    except TimeoutException:
                    ...
                    try:
                        des = wait.until(
                            ec.presence_of_element_located(
                                (
                                    By.XPATH,
                                    "//div[@data-element-name='property-short-description']",
                                )
                            )
                        ).text
                    except TimeoutException:
                    ...
                    try:
                        star = wait.until(
                            ec.presence_of_element_located(
                                (By.XPATH, "//i[@data-selenium='mosaic-hotel-rating']")
                            )
                        ).get_attribute("class")
                    except TimeoutException:
                        star = "non-provided"
