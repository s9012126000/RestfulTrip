    ...
    def hotelcom(self, div):
        ...
        def get_cards():
            ...
            while True:
                ...
                button = WebDriverWait(self.driver, 5).until(
                    ec.element_to_be_clickable(
                        (By.XPATH, "//button[@data-stid='show-more-results']")
                    )
                )
                time.sleep(3)
                button.click()
                ...
                check = len(
                    self.driver.find_elements(
                        By.XPATH, "//section[@class='results']/ol/li"
                    )
                )
                if check > last_len:
                    last_len = check
            ...
            card = self.driver.find_elements(
                By.XPATH,
                "//section[@class='results']/ol/li/div/a[@data-stid='open-hotel-information']",
            )
            return card
        ...
        print(
            f"worker {self.worker_num}: <<{div}>> {len(cards)} cards \n-------------------------------"
        )
        hotel_ls = []
    ...
    def fetch(self, url, n):
        ...
        detail_elm = WebDriverWait(self.driver, 15).until(
            ec.presence_of_element_located(
                (By.XPATH, "//div[@data-stid='content-hotel-title']")
            )
        )
        ...
        try:
            rating = self.driver.find_element(
                By.XPATH, "//div[@itemprop='aggregateRating']"
            ).text
        ...
        try:
            img = (
                self.driver.find_element(By.XPATH, "//div[@id='Overview']")
